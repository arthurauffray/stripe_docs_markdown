...
```curl
curl -X POST https://api.stripe.com/v1/test_helpers/issuing/cards/ic_1MvSieLkdIwHu7ixn6uuO0Xu/shipping/ship \
  -u "<<YOUR_SECRET_KEY>>"
```

```cli
stripe test_helpers issuing cards ship_card ic_1MvSieLkdIwHu7ixn6uuO0Xu
```

```ruby
client = Stripe::StripeClient.new("<<YOUR_SECRET_KEY>>")

card = client.v1.test_helpers.issuing.cards.ship_card('ic_1MvSieLkdIwHu7ixn6uuO0Xu')
```
...
$card = $stripe->testHelpers->issuing->cards->shipCard(
...
CardShipCardParams params = CardShipCardParams.builder().build();

Card card =
  client.v1().testHelpers().issuing().cards().shipCard(
    "ic_1MvSieLkdIwHu7ixn6uuO0Xu",
    params
  );
```
...
Stripe.Issuing.Card card = service.ShipCard("ic_1MvSieLkdIwHu7ixn6uuO0Xu");
```
