...
- `amount` (integer, optional)
  The dispute amount in the card’s currency and in the [smallest currency unit](https://docs.stripe.com/docs/currencies.md#zero-decimal).
...
```cli
stripe issuing disputes update idp_1MykdxFtDWhhyHE1BFAV3osZ \
  -d "evidence[reason]"=not_received \
  -d "evidence[not_received][expected_at]"=1590000000 \
  -d "evidence[not_received][explanation]"= \
  -d "evidence[not_received][product_description]"="Baseball cap" \
...
dispute = client.v1.issuing.disputes.update(
...
$dispute = $stripe->issuing->disputes->update(
  'idp_1MykdxFtDWhhyHE1BFAV3osZ',
  [
    'evidence' => [
      'reason' => 'not_received',
...
Dispute dispute =
  client.v1().issuing().disputes().update("idp_1MykdxFtDWhhyHE1BFAV3osZ", params);
```

```node
const stripe = require('stripe')('<<YOUR_SECRET_KEY>>');

const dispute = await stripe.issuing.disputes.update(
  'idp_1MykdxFtDWhhyHE1BFAV3osZ',
  {
    evidence: {
      reason: 'not_received',
...
params := &stripe.IssuingDisputeUpdateParams{
  Evidence: &stripe.IssuingDisputeUpdateEvidenceParams{
    Reason: stripe.String(stripe.IssuingDisputeEvidenceReasonNotReceived),
    NotReceived: &stripe.IssuingDisputeUpdateEvidenceNotReceivedParams{
      ExpectedAt: stripe.Int64(1590000000),
      Explanation: stripe.String(""),
      ProductDescription: stripe.String("Baseball cap"),
      ProductType: stripe.String(stripe.IssuingDisputeEvidenceNotReceivedProductTypeMerchandise),
    },
  },
}
result, err := sc.V1IssuingDisputes.Update(
...
var options = new Stripe.Issuing.DisputeUpdateOptions
{
    Evidence = new Stripe.Issuing.DisputeEvidenceOptions
    {
        ...
        {
            ExpectedAt = DateTimeOffset.FromUnixTimeSeconds(1590000000).UtcDateTime,
            Explanation = "",
...
Stripe.Issuing.Dispute dispute = service.Update(
    "idp_1MykdxFtDWhhyHE1BFAV3osZ",
    options);
```
