...
```cli
stripe test_helpers issuing transactions create_unlinked_refund  \
  --amount=1000 \
...
$transaction = $stripe->testHelpers->issuing->transactions->createUnlinkedRefund([
  'amount' => 1000,
  'card' => 'ic_1Gswa82eZvKYlo2CP2jveFil',
]);
```
...
TransactionCreateUnlinkedRefundParams params =
  TransactionCreateUnlinkedRefundParams.builder()
    .setAmount(1000L)
...
Stripe.Issuing.Transaction transaction = service.CreateUnlinkedRefund(options);
```
