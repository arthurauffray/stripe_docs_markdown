...
```cli
stripe terminal configurations update tmc_FQqbaQCiy0m1xc \
  -d "bbpos_wisepos_e[splashscreen]"=file_1NtDiHBHO5VeT9SUjuWGkEAN
```
...
$configuration = $stripe->terminal->configurations->update(
  'tmc_FQqbaQCiy0m1xc',
  ['bbpos_wisepos_e' => ['splashscreen' => 'file_1NtDiHBHO5VeT9SUjuWGkEAN']]
);
```
...
ConfigurationUpdateParams params =
  ConfigurationUpdateParams.builder()
    .setBbposWiseposE(
      ConfigurationUpdateParams.BbposWiseposE.builder()
        .setSplashscreen("file_1NtDiHBHO5VeT9SUjuWGkEAN")
        .build()
    )
    .build();

Configuration configuration =
  client.v1().terminal().configurations().update("tmc_FQqbaQCiy0m1xc", params);
```
...
const configuration = await stripe.terminal.configurations.update(
  ...
  {
    bbpos_wisepos_e: {
      splashscreen: 'file_1NtDiHBHO5VeT9SUjuWGkEAN',
    },
  }
);
```
...
params := &stripe.TerminalConfigurationUpdateParams{
  BBPOSWisePOSE: &stripe.TerminalConfigurationUpdateBBPOSWisePOSEParams{
    Splashscreen: stripe.String("file_1NtDiHBHO5VeT9SUjuWGkEAN"),
  },
}
