...
- `documents` (object, optional)
  ...
  - `documents.company_authorization` (object, optional)
    ...
    - `documents.company_authorization.files` (array of strings, optional)
      One or more document ids returned by a [file upload](https://docs.stripe.com/api/persons/create.md#create_file) with a `purpose` value of `account_requirement`.
...
- `verification` (object, optional)
  ...
  - `verification.additional_document` (object, optional)
    ...
    - `verification.additional_document.back` (string, optional)
      The back of an ID returned by a [file upload](https://docs.stripe.com/api/persons/create.md#create_file) with a `purpose` value of `identity_document`. The uploaded file needs to be a color image (smaller than 8,000px by 8,000px), in JPG, PNG, or PDF format, and less than 10 MB in size.

      The maximum length is 500 characters.

    - `verification.additional_document.front` (string, optional)
      The front of an ID returned by a [file upload](https://docs.stripe.com/api/persons/create.md#create_file) with a `purpose` value of `identity_document`. The uploaded file needs to be a color image (smaller than 8,000px by 8,000px), in JPG, PNG, or PDF format, and less than 10 MB in size.
...
```curl
curl https://api.stripe.com/v1/accounts/acct_1032D82eZvKYlo2C/persons \
  -u "<<YOUR_SECRET_KEY>>" \
  -d first_name=John \
  -d last_name=Doe
```
...
```cli
stripe persons create acct_1032D82eZvKYlo2C \
  --first-name=John \
  --last-name=Doe
```
...
Person person =
  client.v1().accounts().persons().create("acct_1032D82eZvKYlo2C", params);
```
...
Person person = service.Create("acct_1032D82eZvKYlo2C", options);
```
