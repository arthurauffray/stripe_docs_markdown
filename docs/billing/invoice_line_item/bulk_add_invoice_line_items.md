...
The updated invoice with newly added line items is returned upon success. Otherwise, this call raises [an error](https://docs.stripe.com/api/invoice-line-item/bulk.md#errors).
...
```cli
stripe invoices add_lines in_1NuhUa2eZvKYlo2CWYVhyvD9 \
  -d "lines[0][description]"="test description" \
  -d "lines[0][amount]"=799 \
  -d "lines[1][invoice_item]"=ii_1NuLVd2eZvKYlo2CRWY0Hqgi
```
...
invoice = client.v1.invoices.add_lines(
...
InvoiceAddLinesParams params =
  InvoiceAddLinesParams.builder()
    ...
    .addLine(
      InvoiceAddLinesParams.Line.builder()
        .setInvoiceItem("ii_1NuLVd2eZvKYlo2CRWY0Hqgi")
        .build()
    )
    .build();

Invoice invoice =
  client.v1().invoices().addLines("in_1NuhUa2eZvKYlo2CWYVhyvD9", params);
```
...
params := &stripe.InvoiceAddLinesParams{
  Lines: []*stripe.InvoiceAddLinesLineParams{
    ...
    &stripe.InvoiceAddLinesLineParams{
      InvoiceItem: stripe.String("ii_1NuLVd2eZvKYlo2CRWY0Hqgi"),
    },
  },
}
result, err := sc.V1Invoices.AddLines(
