...
```cli
stripe quotes update qt_1Mr7wVLkdIwHu7ixJYSiPTGq \
  -d "metadata[order_id]"=6735
```
...
QuoteUpdateParams params =
  QuoteUpdateParams.builder().putMetadata("order_id", "6735").build();

Quote quote = client.v1().quotes().update("qt_1Mr7wVLkdIwHu7ixJYSiPTGq", params);
```
