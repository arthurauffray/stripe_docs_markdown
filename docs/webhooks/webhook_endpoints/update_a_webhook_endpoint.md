...
The updated webhook endpoint object if successful. Otherwise, this call raises [an error](https://docs.stripe.com/api/webhook_endpoints/update.md#errors).
...
- `enabled_events` (array of enums, optional)
...
Possible enum values:
  ...
  - `application_fee.refunded`
    Occurs whenever an application fee is refunded, whether from refunding a charge or from [refunding the application fee directly](https://docs.stripe.com/api/webhook_endpoints/update.md#fee_refunds). This includes partial refunds.
...
```cli
stripe webhook_endpoints update we_1Mr5jULkdIwHu7ix1ibLTM0x \
...
WebhookEndpointUpdateParams params =
  WebhookEndpointUpdateParams.builder()
    .addEnabledEvent(WebhookEndpointUpdateParams.EnabledEvent.CHARGE__SUCCEEDED)
    .addEnabledEvent(WebhookEndpointUpdateParams.EnabledEvent.CHARGE__FAILED)
    .setUrl("https://example.com/new_endpoint")
    .build();

WebhookEndpoint webhookEndpoint =
  client.v1().webhookEndpoints().update("we_1Mr5jULkdIwHu7ix1ibLTM0x", params);
```
