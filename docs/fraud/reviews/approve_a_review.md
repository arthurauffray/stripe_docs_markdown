...
```cli
stripe reviews approve prv_1NVyFt2eZvKYlo2CjubqF1xm
```

```ruby
client = Stripe::StripeClient.new("<<YOUR_SECRET_KEY>>")

review = client.v1.reviews.approve('prv_1NVyFt2eZvKYlo2CjubqF1xm')
```
...
$review = $stripe->reviews->approve('prv_1NVyFt2eZvKYlo2CjubqF1xm', []);
```
...
Review review =
  client.v1().reviews().approve("prv_1NVyFt2eZvKYlo2CjubqF1xm", params);
```

```node
const stripe = require('stripe')('<<YOUR_SECRET_KEY>>');

const review = await stripe.reviews.approve('prv_1NVyFt2eZvKYlo2CjubqF1xm');
```
...
Review review = service.Approve("prv_1NVyFt2eZvKYlo2CjubqF1xm");
```
